...
void DisplayHelp()
{
	...
	printf("write : to write contentws into file \n");
...
int WriteFile(int fd, char *arr, int isize)
{
	if(((UFDTArr[fd].ptrfiletable->mode) != WRITE) && ((UFDTArr[fd].ptrfiletable->mode) !=READ+WRITE))	return -1;
		...
		if(((UFDTArr[fd].ptrfiletable->ptrinode->permission) != WRITE) && ((UFDTArr[fd].ptrfiletable->ptrinode->permission) !=READ+WRITE))	return -1;
			...
			strncpy((UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->readoffset),arr,isize);
...
int CloseFileByNames(char *name)
{
	int i = 0;
...
void ls_file()
{
	...
	printf("\n File Name\tInode number\tFile size\tLinnk count \n");
	printf("-----------------------------------------------------------\n");
	while(temp != NULL)
	{
		if(temp->FileType != 0)
...
int main()
{
	...
	while(1)
	{
		...
		if(count == 1)
		{
			...
			{
				CloseAllFile();
				printf("All files closed succefully\n");
				continue;
			}
		...
		else if(count == 2)
		{
			...
			{
				ret = CloseFileByNames(command[1]);
				if(ret == -1)
					printf("Error : There is no such file\n");
				continue;
			}
